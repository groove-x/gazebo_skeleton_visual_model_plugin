...
class SkeletonModelVisual : public ModelPlugin {
 public:
  void Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if (modelLinkPtr) {
      msgs::Visual actorVisualMsg =
          modelLinkPtr->GetVisualMessage(this->visualName);
      if (actorVisualMsg.has_id()) {
        this->visualId = actorVisualMsg.id();
        gzmsg << "VisualId: " << this->visualId << std::endl;
      } else {
        gzerr << "Message for actor visual [" << modelLinkName << "] not found."
              << std::endl;
      }
    } else {
    ...
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(
        std::bind(&SkeletonModelVisual::OnUpdate, this));
  }
 ...
 private:
  ...
  bool LoadSkin(sdf::ElementPtr _skinSdf, sdf::ElementPtr _targetSdf) {
    ...
    this->AddActorVisual(linkSdf, modelName + "_visual",
    ...
    for (auto iter : nodes) {
      ...
      if (bone->IsRootNode()) pose = ignition::math::Pose3d::Zero;
      ...
      if (bone->IsRootNode()) {
        ...
        this->AddSphereVisual(linkSdf, bone->GetName() + "__SKELETON_VISUAL__",
                              ignition::math::Pose3d::Zero, 0.02, "Gazebo/Red",
      ...
      for (unsigned int i = 0; i < bone->GetChildCount(); ++i) {
        ...
        if (!ignition::math::equal(length, 0.0)) {
          ...
          ignition::math::Pose3d bonePose(
              linkPos, ignition::math::Quaterniond(0.0, phi, theta));
          bonePose.Rot() = pose.Rot().Inverse() * bonePose.Rot();

          this->AddBoxVisual(linkSdf,
                             bone->GetName() + "_" + curChild->GetName() +
                                 "__SKELETON_VISUAL__",
                             bonePose,
  ...
  void PrintTransform(const std::string &_parentFrame, const std::string &_childFrame, const ignition::math::Matrix4d &_transform) {
    ...
    for (unsigned int i = 0; i < this->skeleton->GetNumNodes(); ++i) {
      ...
      if (!parentBone) {
        ...
        physics::LinkPtr parentLink =
            this->model->GetChildLink(parentBone->GetName());
  ...
  //////////////////////////////////////////////////
  void AddSphereInertia(const sdf::ElementPtr &_linkSdf,
                        const ignition::math::Pose3d &_pose, const double _mass,
  ...
  //////////////////////////////////////////////////
  void AddSphereCollision(const sdf::ElementPtr &_linkSdf,
  ...
  //////////////////////////////////////////////////
  void AddSphereVisual(const sdf::ElementPtr &_linkSdf,
                       const std::string &_name,
                       const ignition::math::Pose3d &_pose,
  ...
  //////////////////////////////////////////////////
  void AddBoxVisual(const sdf::ElementPtr &_linkSdf, const std::string &_name,
                    const ignition::math::Pose3d &_pose,
  ...
  //////////////////////////////////////////////////
  void AddActorVisual(const sdf::ElementPtr &_linkSdf, const std::string &_name,
                      const ignition::math::Pose3d &_pose) {
    ...
    meshSdf->GetElement("scale")->Set(ignition::math::Vector3d(
        this->skinScale, this->skinScale, this->skinScale));
  }
};
